...
export default function AdminEditPuppyPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        async function fetchPuppy() {
            const { data, error } = await supabase.from('puppies').select('*').eq('id', resolvedParams.id).single()
            if (data) {
                setFormData({
                    name: data.name,
                    breed: data.breed,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const { error } = await supabase.from('puppies').update(dbPayload).eq('id', resolvedParams.id)
    ...
    if (loading) return <div className="text-gray-500 p-8">Loading puppy details...</div>
    ...
    return (
        <div className="max-w-3xl mx-auto">
            <div className="mb-6 flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-3xl font-serif text-primary-950">Edit Puppy</h1>
                    <p className="text-gray-500 mt-1">Update details for {formData.name}</p>
                </div>
            </div>
